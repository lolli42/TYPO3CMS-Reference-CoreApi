...
*   **Method Injection:** Services retrieved via :ref:`inject*() methods <method-injection>`
    ...
    to be initialized in the constructor. This might change in the future, but it is
    not a high priority.
...
*   **Abstract Classes:** Existing abstract classes that are intended for extension by
    ...
    Extbase's ActionController will not be declared readonly.
...
adopts immutability for newly created constructs and selectively for existing data objects.
